...
<body class="min-h-screen font-poppins" style="background: linear-gradient(to bottom, #DBE9F2 0%, #DBE9F2 45%, #FDE2E4 55%, #FDE2E4 100%); background-attachment: fixed; min-height: 100vh;">
    ...
    @php
        $items = [
            ['route' => 'siswa.dashboard', 'label' => 'Dashboard',        'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z"/>'],
            ['route' => 'siswa.pengajuan', 'label' => 'Ajukan Konseling', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/>'],
            ['route' => 'siswa.kalender',  'label' => 'Kalender Guru BK', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 9v7.5m-9-6h.008v.008H12V12zm0 3h.008v.008H12V15zm0 3h.008v.008H12V18zm-3-6h.008v.008H9V12zm0 3h.008v.008H9V15zm0 3h.008v.008H9V18zm6-6h.008v.008H15V12zm0 3h.008v.008H15V15z"/>'],
            ['route' => 'siswa.jadwal',    'label' => 'Jadwal Konseling',      'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/>'],
            ['route' => 'siswa.riwayat',   'label' => 'Riwayat Konseling',         'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z"/>'],
        ];
        $roleLabel  = 'Siswa';
        $sectionLabel = 'Menu Siswa';
    @endphp
    ...
    <aside id="sidebar-drawer"
        ...
        <nav class="flex-1 py-4 overflow-y-auto px-4">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 mb-3">{{ $sectionLabel }}</p>
    ...
    <div id="main-content" class="transition-all duration-300">
        <header class="sticky top-0 z-30 bg-white border-b border-gray-200 px-5 py-4 flex items-center gap-4 min-h-[57px]">
            ...
            <h1 class="text-gray-800 font-semibold text-base flex-1 truncate">@yield('nav-title', 'Dashboard')</h1>
            {{-- Profile Dropdown --}}
            <div class="relative dd-wrap flex-shrink-0">
                <button onclick="toggleDd('profile-menu-dash')" class="flex items-center gap-2 text-sm focus:outline-none">
                ...
                <div id="profile-menu-dash" class="hidden dd-menu absolute right-0 mt-3 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl py-1 z-50 overflow-hidden text-left origin-top-right transition-all">
                    {{-- User Info Header --}}
                    <div class="px-4 py-3 bg-gray-50/50">
                        <p class="text-sm font-bold text-gray-800 leading-tight truncate">{{ auth()->user()->name }}</p>
                        <p class="text-xs text-gray-500 font-medium leading-tight mt-1 truncate">{{ $roleLabel }}</p>
                    </div>
