...
<body class="min-h-screen font-poppins" style="background: #EBE0E0; background-attachment: fixed;"> 

    <div id="sidebar-overlay" class="hidden fixed inset-0 bg-black/40 z-40" onclick="closeSidebar()"></div>

    <aside id="sidebar-drawer"
        class="fixed inset-y-0 left-0 w-60 bg-[#F8F3F3] border-r border-gray-200 flex flex-col z-50
               -translate-x-full">
        ...
        <nav class="flex-1 py-4 overflow-y-auto px-4">
            ...
            @php
                $navItems = [
                    ...
                    ['route' => 'admin.laporan',   'label' => 'Laporan & Rekap', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>'],
                ];
            @endphp
            ...
            <a href="{{ route($item['route']) }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors mb-1
                      {{ $active ? 'bg-[#EBE0E0]  font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5 flex-shrink-0 {{ $active ? 'text-gray-400' : 'text-gray-400' }}"
    ...
    <div id="main-content" class="transition-all duration-300">
        ...
        {{-- Top bar --}}
        <header class="sticky top-0 z-30 bg-[#F8F3F3] border-b border-gray-200 px-5 py-4 flex items-center gap-4 min-h-[57px]">
