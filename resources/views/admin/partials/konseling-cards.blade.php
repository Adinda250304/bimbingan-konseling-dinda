{{-- Partial: admin/partials/konseling-cards.blade.php --}}
@foreach($items as $k)
@php
    ...
    $tempat   = $k->jadwal ? $k->jadwal->tempat : ($k->jenis === 'online' ? 'Online' : 'Ruang BK');
@endphp
            ...
            <div class="edit-dropdown">
                ...
                <div class="dropdown-menu" id="kdd-{{ $k->id }}">
                    <button class="dropdown-item"
                        onclick="openEditKonseling({{ $k->id }},'{{ $k->status }}','{{ $k->tanggal_konseling ? $k->tanggal_konseling->format('Y-m-d') : '' }}','{{ $k->jam_konseling ? \Carbon\Carbon::parse($k->jam_konseling)->format('H:i') : '' }}');document.getElementById('kdd-{{ $k->id }}').classList.remove('open')">
                        ✏️ Edit Status
                    </button>
                    <a href="{{ route('admin.konseling.show', $k) }}" class="dropdown-item">🔍 Lihat Detail</a>
                </div>
            </div>
        </div>
